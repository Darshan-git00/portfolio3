...
export default function ParticleField() {
  ...
  useEffect(() => {
    ...
    // ── Physics constants — all tuned for smoothness ──────────────────────
    const PARTICLE_COUNT  = 120;
    const MOUSE_RADIUS    = 140;
    const MOUSE_STRENGTH  = 0.018;  // was 0.06 — much gentler pull, no sudden lurch
    const RETURN_SPEED    = 0.008;  // was 0.03 — lazy drift back, not a snap
    const DAMPING         = 0.96;   // was 0.88 — high damping = floaty, no bounce
    const DRIFT_SPEED     = 0.12;   // was 0.18 — slower idle drift
    ...
    const draw = () => {
      ...
      particles.forEach((p, i) => {
        ...
        if (dist < MOUSE_RADIUS && dist > 0) {
          // Smooth quadratic falloff — no sudden jump at radius boundary
        ...
        // Soft spring back to base — lazy, not snappy
        p.vx += (p.baseX - p.x) * RETURN_SPEED;
        p.vy += (p.baseY - p.y) * RETURN_SPEED;

        // High damping — velocity bleeds off slowly = floaty feel
        p.vx *= DAMPING;
        p.vy *= DAMPING;
        ...
        // Base drifts independently of velocity — keeps idle motion alive
        // without coupling it to the spring system (was the jank source)
